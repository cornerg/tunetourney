...
async function fileUploadFn({ tag, url }: uploadData) {
  try {
    ...
    const { data, error } = await supabase.storage.from("user_data").upload(name, file, { upsert: true });
    console.log("Upload response data: ", data);
    if (error) {
      console.error("Error uploading file. ", error, name, file);
      return;
    }
    const uploadedData = supabase.storage.from("user_data").getPublicUrl(data?.path);
...
async function fileDeleteFn({ urls, bucket }: deleteData) {
  ...
  const paths = (Array.isArray(urls) ? urls : [urls]).map((url) => url.match(pathRegex)?.[0]).filter((path) => !!path) as string[];
  const { data, error } = await supabase.storage.from("user_data").remove(paths);
  if (error) {
