...
async function insertSubmissionFn(newEntry: Partial<Submission>) {
  const { data, error } = await supabase.from('Submissions').insert([{ ...newEntry }]).select();
  if (error) {
...
export function useInsertSubmission() {
  const userId = useCurrentUserId();
  return useMutation({
    mutationFn: (submission: Partial<Submission>) => insertSubmissionFn(submission),
...
async function updateSubmissionFn(id: string, submission: Partial<Submission>) {
  const { data, error } = await supabase.from('Submissions').update({ ...submission }).eq('id', id).select();
  if (error) {
...
export function useUpdateSubmission() {
  const userId = useCurrentUserId();
  return useMutation({
    mutationFn: ({ id, ...submission }: InsertParams) => updateSubmissionFn(id, submission),
