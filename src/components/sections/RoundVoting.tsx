...
const dialogueDescription = "Are you sure you want to save your votes as shown? You cannot edit your votes after they've been submitted."
...
export default function RoundVoting({ round }: Props) {
  ...
  const updateVote = React.useCallback(({ submissionId, score, comment }: { submissionId: string; score?: number; comment?: string }) => {
  ...
  const handleSubmit = React.useCallback((votes: StoredVote[], roundId: string, userId: string) => {
    if (!currentUserId) {
  ...
  const canSubmit = React.useMemo(() => {
    ...
    return voteCount === submissionCount && totalScore >= totalScoreRange.min && totalScore <= totalScoreRange.max;
    ...
    <div className="column w-full gap-4">
      <div className="row w-full flex-1 justify-between gap-2">
        ...
        <TTAlertDialogue
          ...
          buttonText="Continue"
          onConfirm={() => handleSubmit(votes, round?.id ?? "", currentUserId ?? "")}
        >
          <TTButton
            buttonStyle="primary"
            className="px-2"
      ...
      <TotalScoreBar min={totalScoreRange.min} max={totalScoreRange.max} limit={totalScoreRange.barLimit} value={currentTotalScore} />
        ...
        {submissions?.map((submission) => {
          ...
          return (
            <SubmissionVote
              ...
              score={vote?.score}
              handleScore={(score: number) => updateVote({ submissionId, score })}
              comment={vote?.comment ?? ""}
              handleComment={(comment: string) => updateVote({ submissionId, comment })}
            />
          )
        })}
      </div>
