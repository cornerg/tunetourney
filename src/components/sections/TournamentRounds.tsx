...
export default function TournamentRounds({ tournamentId }: Props) {
  ...
  const nextRound = React.useMemo(() => {
    return rounds.find((round) => round.status > ROUND_STATUS.pending && round.status < ROUND_STATUS.closed);
  }, [rounds]);
    ...
    <div className="column w-full h-max gap-2 rounded-3xl">
      ...
      {(pendingRounds.length > 0) && (
        <div className="column w-full h-max gap-2 pb-2">
          <div className="row w-full items-center gap-1 cursor-pointer" onClick={() => setExpandUpcoming(!expandUpcoming)}>
            <h4 className="subheading">Upcoming Rounds</h4>
            <RxChevronRight size={22} className="text-dark" style={{ transition: "transform 150ms ease", transform: expandUpcoming ? "rotate(90deg)" : "rotate(0)" }} />
            <hr className="w-full flex-1 text-gray-400" />
          </div>
          <div className="row w-full flex-wrap gap-4 overflow-hidden" style={{ height: expandUpcoming ? "max-content" : 0, overflow: expandUpcoming ? "visible" : "hidden" }}>
      ...
      {(closedRounds.length > 0) && (
        <div className="column w-full h-max gap-2 pb-2">
          <div className="row w-full items-center gap-1 cursor-pointer" onClick={() => setExpandCompleted(!expandCompleted)}>
            <h4 className="subheading">Completed Rounds</h4>
            <RxChevronRight size={22} className="text-dark" style={{ transition: "transform 150ms ease", transform: expandCompleted ? "rotate(90deg)" : "rotate(0)" }} />
            <hr className="w-full flex-1 text-gray-400" />
          </div>
          <div className="row w-full flex-wrap gap-4 overflow-hidden" style={{ height: expandCompleted ? "max-content" : 0, overflow: expandCompleted ? "visible" : "hidden" }}>
