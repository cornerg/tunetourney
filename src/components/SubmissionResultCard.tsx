...
export default function SubmissionResultCard({
  ...
  const totalScore = React.useMemo(
    () => votes?.reduce((total, cur) => total + cur.score, 0),
    [votes],
  );
  ...
  const submitter = React.useMemo(
    () => participants?.find(user => user.id === submission.user_id),
    [participants, submission.user_id],
  );
  ...
  return (
    <div className="relative row w-full h-max pl-6">
      <TTBox className="absolute row w-12 h-12 top-4 left-0 p-0 justify-center items-center z-[2] rounded-full">
        <p
          className={cn("text-center mb-1 leading-none", {
      ...
      <TTBox className="row w-full gap-4 pl-8">
        ...
        <div className="column w-full flex-1 gap-2">
          ...
          <div className="column w-full gap-0">
            <div
              className="row w-full items-center gap-2 cursor-pointer"
              ...
              <div className="column w-full pl-1">
                {sortedVotes.map((vote, i) => {
                  ...
                  return (
                    <div
                      key={vote.id}
                      className={cn("row w-full py-1 pl-1 gap-4", {
                        "border-b border-b-gray-200":
                          i < sortedVotes.length - 1,
                      })}>
                      <div className="row w-max gap-2 items-center min-h-6">
                        <TTTooltip
                          label={voter?.name ?? "Unnamed User"}
                          delay={30}>
                          <ProfilePhoto
                            user={voter}
                            size={24}
                      ...
                      <p className="text-sm h-max pt-0.5">
                        {vote.comment ?? ""}
                      </p>
                    </div>
                  );
