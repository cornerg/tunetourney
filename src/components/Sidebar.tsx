        ...
        {pages.map((page) => {
          ...
          return (
            <div
              key={page.path}
              className={cn(
                "column w-full h-max gap-0 rounded-lg",
                {
                  "shadow": isActive,
                }
              )}>
              ...
              {page.path === "/clubs" && hasClubs && (
                <div className="column w-full h-max pl-2 pb-1.5 gap-0">
                  {sortedClubs.map((club) => {
                    const isSubActive = isActive && page.relativePaths.some((path) => location.pathname === `${path}/${club.id}`);
              ...
              {page.path === "/tournaments" && hasTournaments && (
                <div className="column w-full h-max pl-2 pb-1.5 gap-0">
                  {sortedTournaments.map((tourney) => {
                    const isSubActive = isActive && page.relativePaths.some((path) => location.pathname === `${path}/${tourney.id}`);
